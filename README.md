...
- `login.html` — driver login (username + password).
...
- Each driver has their own username + password; set them from the admin page under Drivers.
...
```sql
ALTER TABLE drivers ADD COLUMN username VARCHAR(100) NOT NULL UNIQUE;
